...
export const Route = createFileRoute("/$countryCode/_public/products/$handle")({
  ...
  head: ({ loaderData }) => {
    ...
    if (!product) {
      return { meta: [{ title: "Product Not Found | ProLift Equipment" }] }
    }
    ...
    const structuredData = {
      ...
      brand: { "@type": "Brand", name: "ProLift Equipment" },
      offers: {
        "@type": "Offer",
    ...
    return {
      meta: [
        ...
        { property: "og:title", content: `${product.title} | ProLift Equipment` },
        { property: "og:description", content: product.description || "Check out this product on ProLift Equipment" },
        { property: "og:image", content: product.thumbnail || "" },
      ],
      links: firstImageUrl ? [{ rel: "preload", href: firstImageUrl, as: "image", fetchPriority: "high" }] : [],
